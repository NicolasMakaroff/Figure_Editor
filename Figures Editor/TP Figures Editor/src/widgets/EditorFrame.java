...
import figures.listeners.transform.MoveShapeTransformListener;
import figures.listeners.transform.RotateShapeTransformListener;
import figures.listeners.transform.ScaleShapeTransformListener;
...
public class EditorFrame extends JFrame implements Signature
{
	...
	public EditorFrame() throws HeadlessException
	{
		...
		fillPaintComboBox.setAlignmentX(Component.CENTER_ALIGNMENT);
		...
		edgePaintComboBox.setAlignmentX(Component.CENTER_ALIGNMENT);
		leftPanel.add(edgePaintComboBox);
		
		JLabeledComboBox edgeTypeCombobox =
			    new JLabeledComboBox("Shape",
			                         LineType.stringValues(),
			                         defaultEdgeTypeIndex,
			                         (ItemListener) null);
			edgeTypeCombobox.setAlignmentX(Component.CENTER_ALIGNMENT);
			edgeTypeCombobox.setPreferredSize(new Dimension(80, 32));
			leftPanel.setLayout(new BoxLayout(leftPanel, BoxLayout.Y_AXIS));
			leftPanel.add(edgeTypeCombobox);
		JLabel edgeWidthLabel =  new JLabel("Line width");
		leftPanel.add(edgeWidthLabel);
		edgeWidthLabel.setAlignmentX(Component.CENTER_ALIGNMENT);
		...
		SpinnerNumberModel snm = new SpinnerNumberModel(defaultEdgeWidth,
		                                                ...
		                                                maxEdgeWidth,
		                                                stepEdgeWidth);
		JSpinner edgeSpinner = new JSpinner(snm);
		leftPanel.add(edgeSpinner);
		
		JSeparator separator_2 = new JSeparator();
		separator_2.setAlignmentY(Component.TOP_ALIGNMENT);
		...
		JCheckBoxMenuItem chckbxmntmEllipse = new JCheckBoxMenuItem("Rectangle");
		chckbxmntmEllipse.setAction(rectangleFilterAction);
		menuFigures.add(chckbxmntmEllipse);
		...
		JCheckBoxMenuItem chckbxmntmCerlce = new JCheckBoxMenuItem("Cercle");
		chckbxmntmCerlce.setAction(circleFilterAction);
		menuFigures.add(chckbxmntmCerlce);
		
		JCheckBoxMenuItem chckbxmntmPolygon = new JCheckBoxMenuItem("Polygon");
		chckbxmntmPolygon.setAction(polyFilterAction);
		menuFigures.add(chckbxmntmPolygon);
		
		JCheckBoxMenuItem chckbxmntmRoundrectangle = new JCheckBoxMenuItem("RoundRectangle");
		chckbxmntmRoundrectangle.setAction(rRectangleFilterAction);
		menuFigures.add(chckbxmntmRoundrectangle);

		JMenu menuColors = new JMenu("Colors");
		menuFilter.add(menuColors);
		
		JCheckBoxMenuItem chckbxmntmBlue = new JCheckBoxMenuItem("FillColor");
		chckbxmntmBlue.setAction(fillColorFilterAction);
		menuColors.add(chckbxmntmBlue);
		
		JCheckBoxMenuItem chckbxmntmEdgecolor = new JCheckBoxMenuItem("EdgeColor");
		chckbxmntmEdgecolor.setAction(edgeColorFilterAction);
		menuColors.add(chckbxmntmEdgecolor);
		...
		// --------------------------------------------------------------------
		moveListener = new MoveShapeTransformListener(drawingModel,history, infoLabel);
		scaleListener = new ScaleShapeTransformListener(drawingModel,history, infoLabel);
		rotateListener = new RotateShapeTransformListener(drawingModel,history, infoLabel);
		selectionListener = new SelectionFigureListener(drawingModel,history, infoLabel,1);
		...
		fillPaintComboBox.addItemListener(new ColorItemListener(fillPaints, defaultFillColorIndex, specialFillColorIndex, PaintToType.FILL));
		
		edgePaintComboBox.addItemListener(new ColorItemListener(edgePaints, defaultEdgeColorIndex, specialEdgeColorIndex, PaintToType.EDGE));
		
		edgeTypeCombobox.addItemListener(new EdgeTypeListener(LineType.fromInteger(edgeTypeCombobox.getSelectedIndex())));
		
		edgeSpinner.addChangeListener(new EdgeWidthListener(defaultEdgeWidth));
	}
	...
	private class FillColorFilterAction extends AbstractAction
	{
		...
		public void actionPerformed(ActionEvent e)
		{
			...
			{
				drawingModel
				    .setFillColorFilter(new FillColorFilter(null));
			}
			else
			{
	...
	private class EdgeColorFilterAction extends AbstractAction
	{
		...
		public void actionPerformed(ActionEvent e)
		{
			...
			{
				drawingModel
				    .setEdgeColorFilter(new EdgeColorFilter());
			}
			else
			{
	...
	private class ShapeItemListener implements ItemListener
	{
		...
		public ShapeItemListener(FigureType type)
		{
			// Mise en place du type de figure
						...
						creationListener =
						    type.getCreationListener(drawingModel,history, infoLabel);
						drawingPanel.addFigureListener(creationListener);
		}
			...
			switch (stateChange)
			{
				case ItemEvent.SELECTED:
				{
					...
					AbstractCreationListener newCreationListener =
					    figureType.getCreationListener(drawingModel,history, infoLabel);
