    ...
    implements Observer, MouseListener, MouseMotionListener, Signature
{
	...
	private void refreshCoordLabel(Point p)
	{
		if ((coordLabel == null) && (p == null)) {
			...
			if(sendInfoState) {
				String stringX = coordFormat.format(p.getX());
				String stringY = coordFormat.format(p.getY());
				coordLabel.setText(" x = " + stringX + " y = " + stringY );
			}
		}
	...
	private void refreshInfoPanel(Point2D p)
	{
		if(((infoPanel != null) ^ sendInfoState)) {
			Figure figure = drawingModel.getFigureAt(p);
			if(figure != null) {
				infoPanel.updateLabels(figure);
			}
			else {
