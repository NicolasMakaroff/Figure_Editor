 ...
 * @author davidroussel
 */
public abstract class Figure implements Prototype<Figure>, Signature
{
	/**
	 * La forme à dessiner
	 */
	...
	/**
	 * Le facteur d'échelle à appliquer à cet objet
	...
	public boolean equals(Object o)
	{
		...
		if (getClass() == o.getClass())
		{
			...
			if (getType().equals(f.getType()))
			{
				...
				{
					// boolean edgeTest = (edge == null ? f.edge == null :
					// edge.equals(f.edge));
					/*
					 * Les egde sont fournies par une PaintFactory donc elles
					 * sont uniques
					 */
					boolean edgeTest = (edge == f.edge);
					if (edgeTest)
					{
						// boolean fillTest = (fill == null ? f.fill == null :
						// fill.equals(f.fill));
						/*
						 * Les fill sont fournies par une PaintFactory donc
						 * elles sont uniques
						 */
						...
						if (fillTest)
						{
							...
							/*
							 * Les stroke sont fournies par une StrokeFactory
	...
	public final void drawSelection(Graphics2D g2D)
	{
		if (selected)
		{
			...
			g2D.drawLine(cx - offset, cy, cx + offset, cy);
			g2D.drawLine(cx, cy - offset, cx, cy + offset);
		}
	}
	...
	public Rectangle2D getBounds2D()
	{
		...
		Point2D[] corners = new Point2D[] { new Point2D.Double(minX, minY),
		    new Point2D.Double(maxX, minY), new Point2D.Double(maxX, maxY),
	...
	{
		/*
		 * Ce point p doit subir la transformation inverse
		...
		catch (NoninvertibleTransformException e)
		{
			System.err.println(getClassName() + "::" + getMethodName()
			    + " contains : Non inversible transformation");
			return false;
		}
	}
	 ...
	 * @param stroke le nouveau {@link BasicStroke} à mettre dans
	 * {@link #stroke}
	 */
	...
	/**
	 * Hashcode of a figure
	 * @return the hash code of the internal Shape (since the equals method will
	 * be based on Shape only in subclasses
	 */
	@Override
	public int hashCode()
	{
		return (shape == null ? 0 : shape.hashCode());
	}

	@Override
	public String toString()
	{
		return getClassName() + " " + String.valueOf(instanceNumber);
	}
}
